...
> **2026-09-01 update (9):** `addmin-app/` — mobile admin CMS app for Zlatica (Android-first, React Native/Expo) — built out through Faze 0-4 (auth, artwork read/status-toggle, full create/edit form with photo upload, Draft Mode preview), each phase verified live against real Sanity data and, where practical, on a physical Android device. Full per-phase trail (what was built, how it was verified, what's still pending a live UI-tap test) lives in `addmin-app/STATUS.md`; plan lives in `addmin-app/docs/`, self-contained and separate from this file. Server-side additions to the main site to support it: `src/lib/admin-api/` (PIN auth, session tokens, admin-scoped Sanity client) and `src/app/api/admin/*` + `src/app/api/preview/*` routes — all server-only, no admin secrets reachable from the public site bundle (verified).
>
> **Two real, pre-existing site bugs found and fixed while building this** (both independent of the admin app itself, surfaced because the Sanity dataset had been empty until now so these paths were never exercised): (1) `works/[slug]/page.tsx` assumed `artwork.medium` always exists, but `medium` isn't a required field in `sanity/schemas/artwork.ts` — any artwork saved without one would have 500'd its own page. Fixed with a GROQ-level fallback (`select(defined(medium) => ..., {fallback})`) in `ARTWORK_FIELDS`, one change covering every call site rather than touching the ~11 places in the codebase that read `artwork.medium.*`. (2) `next.config.ts` never allowlisted `cdn.sanity.io` in `images.remotePatterns` — `next/image` rejected every real Sanity-hosted photo. Added.
>
> Also fixed: root `tsconfig.json` was unintentionally sweeping `addmin-app/` (a separate Expo project with its own `@/*` path aliases) into the main project's `npm run typecheck`, producing false errors. Added `addmin-app` to the root `exclude`.
>
> Not yet done: production `admin-api` env vars (`ADMIN_PIN_HASH`, `ADMIN_SESSION_SECRET`, `SANITY_API_WRITE_TOKEN`) are only set locally for testing — nothing is configured on Vercel yet, so the mobile app only works against a locally-run dev server today, not a real deployed build. See `addmin-app/STATUS.md` for the exact next steps.
>
> **2026-08-30 update (8):** Four owner-requested items shipped together: (1) **Header wordmark** — `Navigation.tsx`'s "ZlaticArt" was fixed at `text-sm` on every breakpoint (never scaled up on desktop); now `clamp(1rem, 1.7vw, 1.5rem)`, plus a slow (7s, mostly-idle) gold shimmer sweep (`.brand-mark` in globals.css) using `background-clip: text`. Hit and fixed a real bug while building it: the initial version set `color: transparent`, which also zeroed out the `currentColor` the gradient's own base stops relied on, so almost the entire wordmark rendered invisible except a thin gold sliver — fixed by using `-webkit-text-fill-color: transparent` instead and leaving `color` (thus `currentColor`) intact. (2) **Hero image already editable** — confirmed `heroCandidate` checkbox on any artwork in Sanity Studio already drives the homepage hero (`sanityGetHeroArtwork()`), already documented in `docs/ADMIN_GUIDE_SR.md`; no code change needed. Noted in passing: `siteSettings.heroArtwork` is a dead/unwired schema field (not read by any query) — left alone, flagged for later cleanup. (3) **Commission request form** — new `/porudzbina` page + form (name, email, format, technique, budget, description), `submitCommissionRequest` server action and `commission_requests` Supabase table/migration, mirroring the existing `/contact` + `contact_submissions` pattern exactly (honeypot, RLS insert-only for `anon`). Linked from the Contact page and site footer. (4) **Footer legal links** — discreet "Privatnost · Autorska prava i uslovi korišćenja" line added below the © line, linking to two new pages, `/privatnost` and `/pravne-informacije`, with real (not fabricated) content grounded in the actual stack: no analytics/tracking cookies (verified by grep — none exist in the codebase), Sanity for content, Supabase for form storage, Vercel hosting.
>
> **Bug found and fixed while QA'ing (1):** screenshots of the new `/porudzbina`, `/privatnost`, `/pravne-informacije` pages showed the header wordmark nearly invisible (only "cAr" of "ZlaticArt" legible). Root cause was unrelated to the shimmer itself: `Navigation` defaults to `theme="dark"` (near-white text) when no `theme` prop is passed, and every one of these content pages (plus several pre-existing ones — `/contact`, `/about`, `/journal`, `/journal/[slug]`, `/studio`, `/works`, `/works/[slug]`) rendered `<Navigation />` with no theme prop despite sitting on a plain light `bg-canvas` background — near-white text on a near-white page, invisible before scroll. This was a **pre-existing site-wide bug**, not something introduced here; the shimmer's gold band just happened to be the only part with enough contrast to show up, which is what surfaced it. Fixed by passing `theme="light"` on all of the above (matching the existing correct pattern already used by `/education` and `/exhibitions`).
>
> Verified: `npm run typecheck` and `npm run build` pass (24 routes). Playwright screenshots confirm the wordmark is now fully legible at every breakpoint and on every page, the shimmer animates without artifacts, the commission form fails gracefully (no console/page errors) since its Supabase table isn't provisioned yet — same pre-deployment state as the existing unshipped contact form — and both new legal pages render with correct content and footer links. Not yet deployed; the `commission_requests` migration needs to be applied to Supabase (alongside the still-pending `contact_submissions` one) before either form can actually store submissions.
>
> **2026-08-30 update (7):** Glass-drag scroll effect (`brushShader.ts`, `HeroGL.tsx`) reworked per direct art-direction feedback, pushed toward near-total abstraction at peak strength — an explicit project-owner decision overriding the general "artwork must stay more memorable than the effect" hero rule (docs/HERO_SPEC.md / living-canvas skill) for this specific scroll effect only, not the initial brush reveal. Changes: (1) **direction fixed** — pigment from *below* a point is pulled *up* into it and mixes with what's above (previously dragged top-down; corrected per feedback), (2) displacement magnitude capped at ~1 UV (one frame height) — an earlier, larger attempt caused most of the 20-tap drag path to sample the same `CLAMP_TO_EDGE` pixel and washed into flat grey fog instead of a real streak, (3) added two-scale path "meander" (fine + coarse noise) so drag taps wander sideways into neighbouring, differently-coloured passages instead of sampling straight up their own column — this is what actually produces visible pigment-mixing at colour boundaries, which was missing entirely before, (4) added a saturation-restore step after the multi-tap average, since blending many genuinely different hues naturally desaturates toward grey/brown — physically correct for real mixed paint, but read as fogging over rather than distinct colours meeting, (5) **`pigmentPullEnvelope()` made monotonic** — ramps in then holds at full strength, no longer fades back to 0 as the pin ends. The fade-back was a real conceptual bug, caught in review: wet dragged paint doesn't un-drag itself just because the viewer keeps scrolling the same direction, so the previous shape made the painting look like it was healing itself right before the handoff, undercutting the entire physical premise. Verified via Playwright screenshots through the scroll pin (0/25/50/75/100%): 0% is the untouched painting; 50/75/100% now all show the same fully-abstracted, colour-mixed state persisting (previously 100% incorrectly reverted to intact). Also verified smoothness by recording a Playwright video through a continuous scroll and sampling frames — the transformation reads as continuous, no popping/jump-cuts between states — and confirmed on a 390×844 touch viewport (mobile's own shorter pin/lower strength still shows the same intact→destroyed progression, no console errors).

(6b) **Mixing direction audit, same day:** caught that the path "meander" added in (3) applied equal-magnitude wander on both axes, so pigment mixing was reading as diagonal/omnidirectional (~1.8:1 vertical:horizontal) rather than the requested strictly-vertical mixing (paint rising and blending with what's above it in roughly the same column). Damped the meander's horizontal component to 28% of its vertical component (~3.2:1 ratio) — re-verified via screenshots that colour transitions now read as clean vertical bands (e.g. a rising orange band blending into yellow, then into a cooler purple above it) rather than diagonal bleed.

`npm run typecheck` and `npm run build` pass throughout. Not yet deployed — sitting on top of the pre-existing uncommitted working-tree pile (see update 4's note); needs a real-browser check before shipping.
>
