...
export default function ExhibitionsPageContent({ exhibitions }: ExhibitionsPageContentProps) {
  const { t } = useLanguage()
  ...
  const statusLabels: Record<Exhibition['status'], string> = {
    upcoming: t.exhibitions.upcoming,
    current: t.exhibitions.current,
    past: t.exhibitions.past,
  }
      ...
      <section className="section-gutter section-spacing" aria-label="Exhibitions list">
        ...
        {upcoming.length > 0 && (
          <div className="mb-20">
            ...
            <ExhibitionList exhibitions={upcoming} statusLabels={statusLabels} moreInfoLabel={t.exhibitions.moreInfo} />
          </div>
        )}
        ...
        {current.length > 0 && (
          <div className="mb-20">
            ...
            <ExhibitionList exhibitions={current} statusLabels={statusLabels} moreInfoLabel={t.exhibitions.moreInfo} />
          </div>
        )}
        ...
        {past.length > 0 && (
          <div className="mb-20">
            <h2 className="text-label text-ink/40 tracking-widest text-xs uppercase mb-10">
              {t.exhibitions.pastSection}
            </h2>
            <ExhibitionList exhibitions={past} statusLabels={statusLabels} moreInfoLabel={t.exhibitions.moreInfo} />
          </div>
        )}

        {exhibitions.length === 0 && (
          <p className="text-ink/40 font-sans text-sm">{t.exhibitions.comingSoon}</p>
        )}

        {exhibitions.length > 0 && upcoming.length === 0 && current.length === 0 && (
          <div className="mb-20 pb-16 border-b border-ink/10">
            <p className="text-ink/40 font-sans text-sm italic">{t.exhibitions.upcomingAnnounced}</p>
          </div>
        )}
      </section>
    </>
...
function ExhibitionList({
  ...
  exhibitions: Exhibition[]
  statusLabels: Record<Exhibition['status'], string>
